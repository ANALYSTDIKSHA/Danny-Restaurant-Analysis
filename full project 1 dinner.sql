...
with cte1 as
(select m.product_name,m.price,s.customer_id,s.order_date,s.product_id , dense_rank() over (partition by customer_id order by order_date asc) as ranks
from sales as s 
join menu as m
on m.product_id = s.product_id) 
...
order by product_name, number_of_times_purchased desc ;


# What is the most purchased item on the menu and how many times was it purchased by each customer?
with cte1 as(select m.product_name,m.price,s.customer_id,s.order_date,s.product_id ,dense_rank() over(order by s.product_id desc) as ranks
from sales as s 
join menu as m
on m.product_id = s.product_id),
...
with cte1 as(
select m.product_name,m.price,s.customer_id,s.order_date,s.product_id ,count(*) as total_units
from sales as s 
join menu as m
on m.product_id = s.product_id
...
from cte1)
select customer_id,product_name,price,total_units
 from cte2 
 where ranks =1 ;
...
# ALTERNATIVE WAY--
with cte1 as(
select me.product_name,me.price,s.customer_id,s.order_date,s.product_id,m.join_date , dense_rank() over(partition by s.customer_id order by s.order_date asc) as ranks
from sales as s
join members as m
on m.customer_id = s.customer_id
join menu as me
...
with cte1 as(
select me.product_name,me.price,s.customer_id,s.order_date,s.product_id,m.join_date , dense_rank() over(partition by s.customer_id order by s.order_date desc) as ranks
from sales as s
join members as m
on m.customer_id = s.customer_id
join menu as me
